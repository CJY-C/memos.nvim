...
| **Create Template** | Instant (~2ms write) + Async (~100ms) | **2** | `POST /api/v1/memos`<br>`PATCH /api/v1/memos/{id}` | Creates the memo with the configured `template_tag` (default `#type/template`), then archives it because the create API does not accept archive state in the create payload. |
...
| **Reset Update Time (`:MemosTimeUpdate2Create`)** | Instant (< 5ms) + Async (~100ms) | **2** | 1x `PATCH /api/v1/memos/{id}`<br>1x `GET /api/v1/memos` (async list refresh) | Sets `update_time` to the selected memo's `create_time`, updates local cache, and refreshes server-side ordering in the background. |
