...
describe("memos.template", function()
	...
	describe("save_template_buffer", function()
		it("should create archived templates in a single create request", function()
			...
			api_mod.Client.create_memo = function(self_api, content, opts, callback)
				create_args = { content = content, opts = opts }
				callback({ name = "memos/template-1", content = content, state = "ARCHIVED" }, nil, {})
			end
			...
			assert.are.same({ state = "ARCHIVED" }, create_args.opts)
			assert.is_false(update_state_called)
			assert.is_true(result.success)
			assert.are.same("memos/template-1", result.new_tpl.name)
		end)

		it("should not delete a memo when archived template create fails", function()
			local api_mod = require("memos.api")
			local old_create_memo = api_mod.Client.create_memo
